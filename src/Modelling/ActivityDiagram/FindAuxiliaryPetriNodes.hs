...
findAuxiliaryPetriNodesConfig'
  FindAuxiliaryPetriNodesConfig
    ...
    | Just high <- snd countOfPetriNodesBounds
    , fst countOfPetriNodesBounds > high =
        Just
          "the second value of 'countOfPetriNodesBounds' must not be smaller than its first value"
    | Just instances <- maxInstances
    , instances < 1 =
        Just
    ...
    , fst (actionLimits adConfig) + forkJoinPairs adConfig < 1 =
        Just
          "The option 'avoidAddingSinksForFinals' can only be achieved if the number of Actions, Fork Nodes and Join Nodes together is positive"
    | otherwise =
        Nothing
...
findAuxiliaryPetriNodesSolution'
  ...
  => p n PetriKey
  -> FindAuxiliaryPetriNodesSolution
findAuxiliaryPetriNodesSolution' petri =
  FindAuxiliaryPetriNodesSolution
...
findAuxiliaryPetriNodesTask path task = do
  ...
  paragraph $ translate $ do
    english
      [i|Translate the given activity diagram into a Petri net (on paper or in your head) and then state the total count of nodes (places and transitions),
the count of auxiliary places and the count of auxiliary transitions in the net.|]
    german
      [i|Übersetzen Sie das gegebene Aktivitätsdiagramm in ein Petrinetz (auf dem Papier oder in Ihrem Kopf) und geben Sie dann die Gesamtanzahl
an Knoten (Stellen und Transitionen), die Anzahl der Hilfsstellen und die Anzahl der Hilfstransitionen des Netzes an.|]
  paragraph $ do
    ...
    translate $ do
      english
        [i|In this example, the resulting net contains 10 nodes in total, of which 2 are auxiliary places and 3 are auxiliary transitions.|]
      german
        [i|In diesem Beispiel etwa enthält das entstehende Netz insgesamt 10 Knoten, davon 2 Hilfsstellen und 3 Hilfstransitionen.|]
    pure ()
...
getFindAuxiliaryPetriNodesTask config@FindAuxiliaryPetriNodesConfig{..} = do
  ...
  ad <-
    mapM (fmap snd . shuffleAdNames) randomInstances
  ...
  where
    checkCount ad =
      let count =
            M.size . Petri.nodes @PetriLike @SimpleNode $
