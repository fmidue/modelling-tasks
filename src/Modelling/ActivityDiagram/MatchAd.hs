...
data MatchAdInstance = MatchAdInstance { activityDiagram :: UMLActivityDiagram
                                       , plantUMLConf :: PlantUmlConfig
                                       , showSolution :: Bool
                                       }
  deriving (Generic, Read, Show)

data MatchAdConfig = MatchAdConfig { adConfig :: AdConfig
...
defaultMatchAdConfig =
  MatchAdConfig { adConfig = defaultAdConfig
...
checkMatchAdConfig conf = checkAdConfig (adConfig conf)
  <|> checkMatchAdConfig' conf

checkMatchAdConfig' :: MatchAdConfig -> Maybe String
checkMatchAdConfig'
  MatchAdConfig { adConfig, maxInstances, noActivityFinalInForkBlocks }
  | isJust maxInstances && fromJust maxInstances < 1 = Just
    "The parameter 'maxInstances' must either be set to a positive value or to Nothing"
  | noActivityFinalInForkBlocks == Just True
    && activityFinalNodes adConfig > 1 = Just
    "Setting the parameter 'noActivityFinalInForkBlocks' to 'Just True' prohibits having more than 1 Activity Final Node"
  | noActivityFinalInForkBlocks == Just False
    && activityFinalNodes adConfig < 1 = Just
    "Setting the parameter 'noActivityFinalInForkBlocks' to 'Just False' requires having at least 1 Activity Final Node"
  | isNothing noActivityFinalInForkBlocks
    && activityFinalNodes adConfig < 1 = Just
    "Having no Activity Final Node means setting the parameter 'noActivityFinalInForkBlocks' to Nothing makes no sense."
  | otherwise = Nothing

matchAdAlloy :: MatchAdConfig -> String
matchAdAlloy MatchAdConfig { adConfig, noActivityFinalInForkBlocks } =
...
data MatchAdSolution =
  MatchAdSolution { actionNodeNames :: [String]
...
matchAdSolution task =
  ...
  in MatchAdSolution { actionNodeNames =
                         sort $ map name $ filter isActionNode $ nodes ad
                     , objectNodeNames =
                         sort $ map name $ filter isObjectNode $ nodes ad
                     , countOfDecisionNodes =
                         length $ filter isDecisionNode $ nodes ad
                     , countOfMergeNodes =
                         length $ filter isMergeNode $ nodes ad
                     , countOfForks = length $ filter isForkNode $ nodes ad
                     , countOfJoins = length $ filter isJoinNode $ nodes ad
                     , countOfInitialNodes =
                         length $ filter isInitialNode $ nodes ad
                     , countOfActivityFinalNodes =
                         length $ filter isActivityFinalNode $ nodes ad
                     , countOfFlowFinalNodes =
                         length $ filter isFlowFinalNode $ nodes ad
                     }

matchAdTask :: (MonadPlantUml m, OutputCapable m)
            => FilePath
            -> MatchAdInstance
            -> LangM m
  ...
  paragraph
    $ do
      translate
        $ do
          english
            [i|To do this, enter your answer as in the following example:|]
...
matchAdInitial =
  MatchAdSolution { actionNodeNames = ["A", "B"]
                  , objectNodeNames = ["C", "D"]
...
matchAdSyntax
  :: OutputCapable m => MatchAdInstance -> MatchAdSolution -> LangM m
matchAdSyntax task sub = addPretext
  $ do
    ...
    assertion (all (`elem` adNames) subNames)
      $ translate
      $ do
...
matchAdEvaluation
  :: OutputCapable m => MatchAdInstance -> MatchAdSolution -> Rated m
matchAdEvaluation task sub = addPretext
  $ do
    let as = translations
          $ do
        ...
        solutionString = if showSolution task
                         then Just $ show sol
                         else Nothing
...
matchAdSolutionMap sol =
  let xs = [ Left $ sort $ actionNodeNames sol
...
matchAd :: (MonadAlloy m, MonadThrow m)
        => MatchAdConfig
        -> Int
        -> Int
        -> m MatchAdInstance
...
getMatchAdTask :: (MonadAlloy m, MonadThrow m, RandomGen g)
               => MatchAdConfig
               -> RandT g m MatchAdInstance
getMatchAdTask config = do
  alloyInstances <- getInstances (maxInstances config) Nothing
    $ matchAdAlloy config
  ...
  return
    $ MatchAdInstance { activityDiagram = ad
                      , plantUMLConf =
                          defaultPlantUmlConfig { suppressBranchConditions =
                                                    hideBranchConditions config
                                                }
...
defaultMatchAdInstance =
  MatchAdInstance { activityDiagram =
                      UMLActivityDiagram { nodes =
                                             [ AdActionNode { label = 1
                                                            , name = "A"
                                                            }
                                             , AdActionNode { label = 2
                                                            , name = "B"
                                                            }
                                             , AdActionNode { label = 3
                                                            , name = "C"
                                                            }
                                             , AdActionNode { label = 4
                                                            , name = "D"
                                                            }
                                             , AdObjectNode { label = 5
                                                            , name = "E"
                                                            }
                                             , AdObjectNode { label = 6
                                                            , name = "F"
                                                            }
                                             , AdObjectNode { label = 7
                                                            , name = "G"
                                                            }
                                             , AdObjectNode { label = 8
                                                            , name = "H"
                                                            }
                                         ...
                                         , connections =
                                             ...
                                             , AdConnection { from = 11
                                                            , to = 13
                                                            , guard = ""
                                                            }
                                             , AdConnection { from = 12
                                                            , to = 6
                                                            , guard = ""
                                                            }
                                             , AdConnection { from = 13
                                                            , to = 1
                                                            , guard = ""
                                                            }
                                             , AdConnection { from = 13
                                             ...
                                             , AdConnection { from = 14
                                                            , to = 15
                                                            , guard = ""
                                                            }
                                             , AdConnection { from = 17
                                                            , to = 4
                                                            , guard = ""
                                                            }]
                                         }
