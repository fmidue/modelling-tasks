FindInstance 
  ...
  , toFind = Conflict { conflictTrans = ( Transition 3, Transition 4 ), conflictPlaces = [ Place 4 ] }
  , net = PetriLike 
      { allNodes = fromList
          [ ( "s1"
            , SimplePlace { initial = 0, flowOut = fromList [ ( "t5", 1 ) ] }
            )
          , ( "s2"
            , SimplePlace 
                { initial = 2
          ...
          , ( "s3"
            , SimplePlace { initial = 1, flowOut = fromList [ ( "t3", 1 ) ] }
            )
          , ( "s4"
            , SimplePlace 
                { initial = 2
                , flowOut = fromList [ ( "t3", 2 ), ( "t4", 2 ) ]
                } 
            )
          , ( "s5"
            , SimplePlace { initial = 0, flowOut = fromList [ ( "t5", 1 ) ] }
          ...
          , ( "t4", SimpleTransition { flowOut = fromList [ ( "s5", 1 ) ] } )
